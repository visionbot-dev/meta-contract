...
async function main() {
  ...
  // 池内 LP 储备 = 当前池创建交易输出 3 的 LP 金额（池地址上可能还有用户预存的 LP，不能误当储备）
  const poolTxObj = new mvc.Transaction(poolTxHex)
  const lpReserveScriptBuf = poolTxObj.outputs[3].script.toBuffer()
  const lpReserveAmount = ftProto.getTokenAmount(lpReserveScriptBuf).toString()
  const lpListAll = await getFtUtxos(poolAddrStr, TOKENS.LP.codehash, TOKENS.LP.genesis)
  const lpR = lpListAll.find((x) => x.valueString === lpReserveAmount)
  if (!lpR) throw new Error(`no pool LP reserve (amount=${lpReserveAmount}) at pool address`)
